...
( function( $, wpLinkL10n, wp ) {
	...
	window.wpLink = {
		...
		mceRefresh: function( searchStr, text ) {
			...
			if ( linkNode.length ) {
				...
				if ( href !== '_wp_link_placeholder' ) {
					...
					inputs.noFollow.prop( 'checked', linkNode.attr( 'rel' ).split( ' ' ).indexOf( 'no-follow' ) !== -1 );
      ...
      var url = $( '#wp-link-url' ),
          newTab = $( '#wp-link-target' ),
          noFollow = $( '#wp-link-nofollow' );

      return {
        href: $.trim( url.val() ),
        target: newTab.prop( 'checked' ) ? '_blank' : null,
        rel: noFollow.prop( 'checked' ) ? 'no-follow' : null
      };
		},
	...
	$.extend( River.prototype, {
		...
		process: function( results, params ) {
          ...
          var type = this.hasOwnProperty('type') ? this.type : 'post',
              extraClass = !document.querySelector('.filter-results').value || document.querySelector('.filter-results').value == type ? 'show' : '';

					classes = alt ? 'alternate' : '';
          classes += ' ' + type + '-item ' + extraClass;
