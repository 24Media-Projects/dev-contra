...
$term_ids = get_field('btw__group_fields__magazine__template__articles_grid_three_cols__categories', $group_id);

if( !$term_ids ) return;

global $btw_log_posts;

$three_cols_posts = new WP_Query([
 ...
 */
$btw_log_posts->log_posts( wp_list_pluck( $three_cols_posts->get_posts(), 'ID' ) );
...
?>
<section id="group-<?php echo $group_id; ?>" class="articles_grid_three_cols_section <?php echo $section_extra_class ?? ''; ?> <?php if( $section_title || $related_links ) echo 'magazine_tribute'; ?> <?php if( $is_dark_mode ) echo 'section_darkmode'; ?>"
    ...
    <div class="three_articles_grid">
        <?php while( $three_cols_posts->have_posts() ){
		$three_cols_posts->the_post();
