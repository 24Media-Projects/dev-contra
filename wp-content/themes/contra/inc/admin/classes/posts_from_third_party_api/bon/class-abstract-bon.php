...
abstract class Bon{
    ...
    abstract public static function schedule_event();


    abstract protected function get_feeds();


    abstract protected function get_group_ids();
        ...
        foreach( $this->feeds as $feed ){
            ...
            foreach( $feed['urls'] as $feed_url ){
                ...
                if( is_wp_error( $response ) ){
                    error_log( 'BON Request Error: ' . $response->get_error_message() . "\n" . ' Feed Url: '  . $feed_url );
                    continue;
                }

                if( wp_remote_retrieve_response_code( $response ) !== 200 ){
                    error_log( 'BON Request HTTP Error: ' . ( $response['response']['code'] ?? 'Undefined' ) . "\n" . ' Feed Url: '  . $feed_url );
                    continue;
                }
                ...
                foreach( $groups as $group ){
                    ...
                    $group_posts = array_map( function( $group_post ) use( $publication_name, $feed_section, $publication_url ){
                        ...
                        $post_attributes = [
                            ...
                            'post_impression_url'   => '',
                            'post_primary_category' => $post['primary_category'],
                        ];
            ...
            // publications that are not in wp
                ...
                $posts = array_map( function( $post ) use( $feed_section, $publication_name, $publication_url ) {
                    ...
                    $post_attributes = [
                        ...
                        'post_publication_name' => $publication_name,
                        'post_publication_url'  => $publication_url,
                        'post_impression_url'   => $post['impressionurl'] ?? '',
                        'post_primary_category' => $post['homeSectionItem']['label'] ?? '',
                    ];
