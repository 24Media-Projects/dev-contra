...
$pname = $primary_tag->name;
?>

<div class="tag_related_articles">
	<label class="label s-font-bold">ΠΕΡΙΣΣΟΤΕΡΑ:</label>
    <!-- <span class="tag_name s-font-bold"><?php echo  remove_punctuation($pname); ?> </span> -->
    <ul class="articles_list">

        <?php while( $primary_tag_query->have_posts() ): 
        	$primary_tag_query->the_post(); 

        	$title = get_the_title();
        ?>
        <li class="suggested-article-title">
            <a href="<?php the_permalink(); ?>" title="<?php echo $title; ?>">
            	<?php echo remove_punctuation($title); ?>
            </a>
        </li>
        <?php endwhile;
		wp_reset_query();
